...
def remove_outer_indent(lines):
    min_indent = reduce(lambda x, y: min(x, len(y) - len(y.lstrip())), lines, inf)

    return [l[min_indent:] for l in lines]


def safe_open_weights(weights_file):
    """Try to open weights, restore from .bak if corrupted"""
    try:
        return shelve.open(weights_file)
    except Exception as e:
        if "truncated" in str(e).lower() or "corrupt" in str(e).lower():
            log.warning(f"Weights file corrupted, restoring from backup: {e}")
            # Simple restore: copy .bak to .dat
            import shutil

            bak_file = weights_file + ".bak"
            dat_file = weights_file + ".dat"
            if os.path.exists(bak_file):
                shutil.copy2(bak_file, dat_file)
                return shelve.open(weights_file)
        raise
...
class Quiz:
    ...
    def create_questions(self):
        ...
        def grep(e):
            if not configuration.g:
                return True
            return any(
                pattern in line for pattern in configuration.g for line in e["q"]
            )
        ...
        if configuration.m:
            for m in configuration.m:
                if m.startswith((".", "/")):
                    module = imp.load_source("dyn" + str(dynamic_module_count), m)
    ...
    def ask(self):
        ...
        self.current_q_index = None
        question_weights = safe_open_weights(weights_file)
    ...
    def __init__(self, lines, name):
        ...
        for chunk in groupby(enumerate(lines), is_comment):
            if chunk[0]:
                ...
                q = {
                    "q": remove_outer_indent(
                        [remove_comment(c[1]).rstrip() for c in q_lines]
                    ),
                    "ln": q_lines[0][0] + 1,
                }
                if name:
